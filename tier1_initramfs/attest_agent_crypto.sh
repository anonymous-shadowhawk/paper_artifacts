...
generate_aik() {
    ...
    if ! command -v openssl >/dev/null 2>&1; then
        error "OpenSSL not available - cannot generate real keys"
        return 1
    fi
    
    if ! openssl genrsa -out "$priv_key" "$KEY_SIZE" 2>/dev/null; then
        error "Failed to generate RSA private key"
        return 1
    fi
    
    if ! openssl rsa -in "$priv_key" -pubout -out "$pub_key" 2>/dev/null; then
...
EOF
    ...
    if ! openssl dgst -${HASH_ALG} -sign "$priv_key" -out "$quote_sig" "$quote_data" 2>/dev/null; then
...
EATEOF
    ...
    if [ -f "$eat_file" ]; then
        log " EAT token created with real cryptographic signatures"
        return 0
    else
